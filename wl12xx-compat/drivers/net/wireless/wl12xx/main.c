...
#define WL12XX_NVS_FAC "ti-connectivity/wl12xx-fac-nvs.bin"

static int wl1271_fetch_nvs_nowait(struct wl1271 *wl, char * nvsFile);
//END IKJBREL2-2970
...
static void wl12xx_list_add(struct wl1271 *wl)
{
	mutex_lock(&wl_list_mutex);
	if (!wl->on_list)
		list_add(&wl->list, &wl_list);
	else
		wl1271_error("%s: already on list", __func__);
	wl->on_list = true;
	mutex_unlock(&wl_list_mutex);
}

static void wl12xx_list_del(struct wl1271 *wl)
{
	mutex_lock(&wl_list_mutex);
	if (wl->on_list)
		list_del(&wl->list);
	else
		wl1271_error("%s: not on list", __func__);
	wl->on_list = false;
	mutex_unlock(&wl_list_mutex);
}
				      ...
				      struct ieee80211_vif *vif)
{
	...
	/* if the request is not valid, fake a stopped event */
	if (wl->sched_vif == wlvif)
		wl1271_scan_sched_scan_stop(wl, wlvif);
	else
		ieee80211_sched_scan_stopped(wl->hw);
...
static int wl12xx_probe(struct platform_device *pdev)
{
	...
	//BEGIN a19110 IKJBREL2-2970
	ret = wl1271_fetch_nvs_nowait(wl,WL12XX_NVS_FAC);
	//END IKJBREL2-2970
	if (ret)
		goto out_free_hw;
